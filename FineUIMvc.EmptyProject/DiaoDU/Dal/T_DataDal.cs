...
namespace Dal
{
    public class T_DataDal
    {
        static String tbname = "T_Data" + DateTime.Now.ToString("yyyyMM");
                 
        public static Hashtable Search(string key, int index, int size, string sortField, string sortOrder)
        { 
            DataTable dt = DBUtil.SelectDataTablePager(key, index, size);
            ArrayList data = DBUtil.DataTable2ArrayList(dt);

            int count = 0;//DBUtil.ExecuteScalar(sql);
