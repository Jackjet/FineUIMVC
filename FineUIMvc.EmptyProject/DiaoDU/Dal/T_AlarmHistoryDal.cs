...
namespace Dal
{
    public class T_AlarmHistoryDal
    {
        static String tbname = "T_AlarmHistory";

        public static Hashtable Search(string key, int index, int size, string sortField, string sortOrder)
        {
            string sql = @"SELECT   PumpManager.FName, T_AlarmHistory.*  FROM T_AlarmHistory INNER JOIN PumpManager ON T_AlarmHistory.FPumpID = PumpManager.ID  where 1=1 " + key + "";
