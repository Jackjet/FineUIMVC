$(function () {
    ...
    function dealField(result) {
        ...
        for (var i = 0; i < result.length; i++) {
            ...
            if (result[i].FCommon == 1 || result[i].FCommon == 4) {
                var iClass = (result[i].IsSelect2 == 1 ? 'checked' : '');
    ...
    //EditParmSelect1
    var isSelected = 0;
    $(".main_wrap").on("click", "i", function () {
        var _that = $(this);
        var originSelect = 0;
        var opFieldId = $(this).parent("li").attr("data-id");
        if ($(this).parent("li").attr("data-type") == 2 || $(this).parent("li").attr("data-type") == 4) {
            console.log("不可更改状态");
            return;
        } else {
            if ($(this).hasClass("checked")) {
