...
namespace FineUIMvc.PumpMVC.Common
{
    public static class PagingSorting
    {
        ...
        //排序
        public static IQueryable<T> Sort<T>(IQueryable<T> q, string SortField, string SortDirection)
        {
            return q.SortBy(SortField + " " + SortDirection);
        }

        // 排序后分页
        public static IQueryable<T> SortAndPage<T>(IQueryable<T> q, int PageIndex, int PageCount, int PageSize, string SortDirection, string SortField)
        {
